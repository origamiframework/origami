package ru.origami.hibernate.log4jdbc.sql.jdbcapi;

import ru.origami.hibernate.log4jdbc.log.SpyLogDelegator;
import ru.origami.hibernate.log4jdbc.sql.Spy;
import ru.origami.hibernate.log4jdbc.sql.Utilities;
import ru.origami.hibernate.log4jdbc.sql.jdbcapi.ConnectionSpy;
...
public class StatementSpy implements Statement, Spy
{
	 ...
	 */
	protected ru.origami.hibernate.log4jdbc.sql.jdbcapi.ConnectionSpy connectionSpy;
	...
	/**
	 * Conveniance method to report (for logging) that a method returned long array.
	 *
	 * @param methodCall description of method call and arguments passed to it that returned.
	 * @param value long array return value.
	 * @return the long array as passed in.
	 */
	protected long[] reportReturn(String methodCall, long[] value)
	{
		reportAllReturns(methodCall, Arrays.toString(value));
		return value;
	}
	...
	protected ResultSet getGeneratedKeys(String sql) throws SQLException {
		...
		try {
			...
			return (ResultSet) reportReturn(methodCall, new ru.origami.hibernate.log4jdbc.sql.jdbcapi.ResultSetSpy(this, r, this.log));
		} catch (SQLException s) {
...
//			}
	...
	public ResultSet executeQuery(String sql) throws SQLException {
		...
		try {
			...
			reportStatementSqlTiming(System.currentTimeMillis() - tstart, sql, methodCall);
			ru.origami.hibernate.log4jdbc.sql.jdbcapi.ResultSetSpy r = new ru.origami.hibernate.log4jdbc.sql.jdbcapi.ResultSetSpy(this, result, this.log);
			return (ResultSet) reportReturn(methodCall, r);
		} catch (SQLException s) {
			reportException(methodCall, s, sql, System.currentTimeMillis() - tstart);
			throw s;
	...
	public long getLargeUpdateCount() throws SQLException {
		...
		try {
			return reportReturn(methodCall,realStatement.getLargeUpdateCount());
		} catch (SQLException s) {
			reportException(methodCall,s);
			throw s;
		}
	...
	public long getLargeMaxRows() throws SQLException {
		...
		try {
			return reportReturn(methodCall,realStatement.getLargeMaxRows());
		} catch (SQLException s) {
			reportException(methodCall,s);
			throw s;
		}
	...
	public long[] executeLargeBatch() throws SQLException {
		...
		try {
			updateResults = realStatement.executeLargeBatch();
			reportSqlTiming(System.currentTimeMillis()-tstart, sql, methodCall);
		} catch (SQLException s) {
			reportException(methodCall, s, sql, System.currentTimeMillis()-tstart);
			throw s;
