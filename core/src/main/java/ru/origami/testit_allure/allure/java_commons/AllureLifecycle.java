...
import ru.origami.testit_allure.allure.java_commons.internal.AllureStorage;
import ru.origami.testit_allure.allure.java_commons.internal.AllureThreadContext;
import ru.origami.testit_allure.allure.java_commons.listener.*;
import ru.origami.testit_allure.allure.java_commons.util.PropertiesUtils;
import ru.origami.testit_allure.allure.model.*;
import ru.origami.testit_allure.allure.model.Attachment;
...
import static ru.origami.testit_allure.allure.java_commons.AllureConstants.ATTACHMENT_FILE_SUFFIX;
import static ru.origami.testit_allure.allure.java_commons.util.ServiceLoaderUtils.load;
...
//            LOGGER.error("Could not update step: no step running");
    ...
    public void updateStep(final String uuid, final Consumer<StepResult> update) {
        ...
        if (!found.isPresent()) {
            LOGGER.error("ALLURE Could not update step: step with uuid {} not found", uuid);
            return;
        }
    ...
    public void stopStep() {
        ...
        if (!current.isPresent()) {
//            LOGGER.error("ALLURE Could not stop step: no step running");
            return;
        }
        final String uuid = current.get();
        stopStep(uuid);
