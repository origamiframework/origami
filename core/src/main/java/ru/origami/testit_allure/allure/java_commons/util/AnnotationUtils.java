...
import ru.origami.testit_allure.allure.java_commons.Flaky;
import ru.origami.testit_allure.allure.java_commons.LabelAnnotation;
import ru.origami.testit_allure.allure.java_commons.Muted;
import ru.origami.testit_allure.allure.model.Label;
import ru.origami.testit_allure.allure.model.Link;
...
import static ru.origami.testit_allure.allure.java_commons.util.ResultsUtils.createLabel;
import static ru.origami.testit_allure.allure.java_commons.util.ResultsUtils.createLink;
...
public final class AnnotationUtils {
    ...
    /**
     * Returns true if {@link Flaky} annotation is present.
     *
     * @param annotatedElement the element to search annotations on.
     * @return true if {@link Flaky} annotation is present, false otherwise.
     */
    ...
    /**
     * Returns true if {@link Muted} annotation is present.
     *
     * @param annotatedElement the element to search annotations on.
     * @return true if {@link Muted} annotation is present, false otherwise.
     */
    ...
    private static Stream<Link> extractLinks(final LinkAnnotation m, final Annotation annotation) {
        ...
        if (annotation instanceof ru.origami.testit_allure.annotations.Link) {
            return Stream.of(createLink((ru.origami.testit_allure.annotations.Link) annotation));
        }

        if (Objects.equals(m.value(), LinkAnnotation.DEFAULT_VALUE)) {
            return callValueMethod(annotation)
                    .map(value -> createLink(value, null, m.url(), m.allureType()));
        }
        return Stream.of(createLink(m.value(), null, m.url(), m.allureType()));
    }
